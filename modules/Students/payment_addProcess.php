...
$hasFinanceAccess = $staffGateway->hasPermission($staffID, 'payments.create');
...
try {
    ...
    $paymentType = filter_var($_POST['paymentType'] ?? '', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    if (!in_array($paymentType, ['program', 'other'])) {
    ...
    // Prepare payment data matching actual database schema
    $paymentData = [
        'studentID' => $studentID,
        'amount' => $amount,
    ...
    // For program payments, create charge allocation
    if ($paymentType === 'program') {
        $paymentData['appliedToCharges'] = json_encode([
            [
                'chargeType' => 'tuition',
                'amount' => $amount,
                'description' => 'Program payment allocation'
            ]
        ]);
    }
    ...
    if ($paymentID) {
        ...
        $_SESSION['flash_errors'] = ['Failed to record payment'];
        header("Location: index.php?q=/modules/Students/student_manage_view.php&studentID=$studentID&tab=bursar");
        exit;
    }
